        ...
        <div class="row mb-4">
            <div class="col-md-3">
                <div class="stats-card">
                    ...
                    <div class="stats-label">Total Permintaan</div>
                </div>
            </div>
            <div class="col-md-3">
                <div class="stats-card">
        ...
        <div class="table-responsive">
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        <td><span class="procurement-code">{{ $pmt->code }}</span></td>
                        <td><span class="item-count">{{ $pmt->pegawai->nip }}</span></td>
                        <td><span class="item-count">{{ $pmt->regarding }}</span></td>
                        <td><span class="item-count">{{ $pmt->sifat }}</span></td>
                        <td><span class="item-count">{{ $pmt->total_item }}</span></td>
                        <td>
                            @if ($pmt->status == 'Selesai')
                                <span class="status-badge status-approved">{{ $pmt->status }}</span>
                            @elseif ($pmt->status == 'Menunggu')
                                <span class="status-badge status-pending">{{ $pmt->status }}</span>
                            @elseif ($pmt->status == 'Ditolak')
                                <span class="status-badge status-rejected">{{ $pmt->status }}</span>
                            @endif
                        </td>
                        <td><span class="date-cell">{{\Carbon\Carbon::parse($pmt->created_at)->format('d-m-Y')}}</span></td>
                        <td>
